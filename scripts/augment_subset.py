...
# - Saves augmented WAVs into data_aug/{speech,noise} and logs a manifest.csv (reproducibility).
...
# or with options:
#     python scripts/augment_subset.py --src data --out data_aug --n_speech 5000 --n_noise 5000 --sr 16000
# """
...
# def random_bandlimit(x: np.ndarray, sr: int) -> np.ndarray:
#     """
#     Random simple coloration with safe cutoffs and SOS filtering:
#       - 40%: band-pass in a speechy band
#       - 30%: high-pass (remove lows)
#       - 30%: low-pass (remove highs)
#     """
#     r = random.random()
#     nyq = sr / 2.0
...
#     elif r < 0.70:
#         # High-pass
#         cut_hz = random.uniform(80.0, 400.0)
...
#     else:
#         # Low-pass
#         cut_hz = random.uniform(2500.0, nyq - 200.0)  # margin from Nyquist
#         wn = _safe_norm(cut_hz, sr)
#         sos = _butter_sos(4, wn, btype="lowpass")
#         return sig.sosfiltfilt(sos, x)

# def apply_reverb(x: np.ndarray, sr: int) -> np.ndarray:
#     """Short 'roomy' reverb via exponential IR; normalized safely to avoid div-zero."""
#     decay = random.uniform(0.2, 0.4)        # seconds
...
# def shift_signal(x: np.ndarray, sr: int):
#     """Circular-ish shift with zero padding to keep length constant."""
#     max_shift = int(0.2 * sr)  # ±200 ms
...
# def mix_with_noise(speech: np.ndarray, noise: np.ndarray, snr_db: float) -> np.ndarray:
#     """Scale noise to target SNR wrt (possibly augmented) speech; mix and peak-normalize."""
#     sp = speech
#     ns = noise
#     sp_rms = rms(sp)
#     ns_rms = rms(ns)
#     desired_ns_rms = sp_rms / (10.0 ** (snr_db / 20.0))
#     ns_scaled = ns * (desired_ns_rms / (ns_rms + 1e-12))
#     y = sp + ns_scaled
...
#     parser.add_argument("--out", type=str, default="data_aug", help="Output root for augmented data")
...
#         "outfile","source","class","gain_db","snr_db","shift_ms","reverb","bandlimit"
#     ])
#     writer.writeheader()
...
#         band_used = False
#         if random.random() < 0.3:
#             x = random_bandlimit(x, SR)
#             band_used = True

#         snr_db = random.choice([-10, -5, 0, 5, 10])
#         noise = load_random_noise(all_noise_segments, SR, TARGET_LEN)
#         y = mix_with_noise(x, noise, snr_db)
...
#         bins = np.arange(-12, 12+2, 2)
#         plt.hist(snr_values, bins=bins, edgecolor="black")
#         plt.title("SNR Distribution (Speech Augmentations)")
...
#         plt.title("Gain Distribution (All Augmentations)")
